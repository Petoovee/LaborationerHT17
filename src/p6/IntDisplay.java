...
public class IntDisplay extends JComponent {
	...
	private void init(int background, int grid, Mode mode, int textColor) {
		...
		setPreferredSize(new Dimension(PAGE_SIZE * (gridWidth + gridStroke) + gridStroke,
				PAGE_SIZE * (gridHeight + gridStroke) + gridStroke));
	...
	protected void paintComponent(Graphics g) {
		...
		for (int row = 0; row < systemValues.length; row++) {
			for (int col = 0; col < systemValues[row].length; col++) {
				...
				dBufferG.fillRect(offsetX + gridStroke + col * (width + gridStroke),
						offsetY + gridStroke + row * (height + gridStroke), width, height);
