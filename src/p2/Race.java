...
public class Race {
	...
	public void action() {
		...
		// int space = window.getBackgroundHeight(); //Holds a temporary value for
		// referencing where the current height limit is
		...
		for (int i : carNbr) // A makeshift ticker to keep track of which car is which!
		{
			...
			car[i].moveTo(window.getBackgroundWidth() - car[i].getImage().getIconWidth(),
					window.getBackgroundHeight() - (window.getBackgroundHeight() / nbrOfCars) * (i + 1)); // Positions
																											// each car
