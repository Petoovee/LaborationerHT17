...
public class Game {
	...
	public void play(boolean showThrows) {
		...
		if (p1 > p2) {
			TextWindow.println(player1.getName() + " vann �ver " + player2.getName() + "  " + p1 + "-" + p2);
		} else if (p2 > p1) {
			TextWindow.println(player2.getName() + " vann �ver " + player1.getName() + "  " + p2 + "-" + p1);
		} else {
			TextWindow.println(
					"Oavgjort mellan " + player1.getName() + " och " + player2.getName() + "  " + p1 + "-" + p2);
		}
	}
}
