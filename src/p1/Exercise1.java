...
{	
	...
	{
		long deadbeats, squeakers, oldies, total;   //Behˆver ju inte begr‰nsas med lille int
		double pris, genomsnittspris;   //Decimaler! Eller bara hˆga priser?
		...
		genomsnittspris = (double) Math.round((pris / total) * 100) / 100;   // Fˆr att skriva ut de genomsnittliga kostnaden med tvÂ decimalers precision
		JOptionPane.showMessageDialog(null, "The total price for all " + total + " passengers is " + pris + " averaging to " + genomsnittspris + " per \"person\"");
	}
...
//			PaintWindow.pause(1); // pausa exekveringen i 20 ms innan n√§sta f√∂rflyttning
			x += dx;
			y += dy;
			if(x<0 || x>500) 
			{
